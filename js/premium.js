/* =========================================================
   Taiseer Transactions Services — PREMIUM (Vibrant) JS
   ========================================================= */
(function () {
  'use strict';
  ...
  if (grid) {
    grid.innerHTML = SERVICES.map(function (s, i) {
      ...
      return '<article class="scard reveal" data-delay="' + (i % 4) + '">' +
        '<div class="scard__icon ' + t + '">' + (ICON[s.icon] || '') + '</div>' +
        '<h4>' + s.title + '</h4><p>' + s.desc + '</p>' +
        '<a class="scard__arrow" href="https://wa.me/' + WHATSAPP + '?text=' +
          encodeURIComponent('Hello Taiseer Typing, I need: ' + s.title) +
          '" target="_blank" rel="noopener">Enquire ' + arrow + '</a>' +
        '</article>';
    }).join('');
  }
